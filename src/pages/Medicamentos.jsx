...
import { getMedicamentos, createMedicamento, deleteMedicamento, updateMedicamento } from "../api/medicamentoApi";
...
export default function Medicamentos() {
  ...
  const [form, setForm] = useState({ nombre: "", descripcion: "", dosis: "" });
  ...
  const submit = async (e) => {
    ...
    setForm({ nombre: "", descripcion: "", dosis: "" });
    await load();
  };
        ...
        <main className="p-6 flex-1 relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <motion.form onSubmit={submit} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7 }} className="col-span-1 p-6 rounded-2xl bg-black/40 border border-pink-500/30 shadow-[0_0_20px_pink] backdrop-blur-xl space-y-3">
              ...
              <input value={form.dosis} onChange={e => setForm({ ...form, dosis: e.target.value })} placeholder="Dosis" className="w-full p-2 rounded bg-white/5 border border-white/10"/>
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7 }} className="col-span-2 p-6 rounded-2xl bg-black/40 border border-[#65BDB1]/30 shadow-[0_0_20px_#65BDB155] backdrop-blur-xl">
              <h2 className="font-semibold mb-4 text-[#65BDB1]">Listado de Medicamentos</h2>
              <div className="space-y-3 max-h-[60vh] overflow-auto pr-2">
                {items.map(m => (
                  <motion.div key={m._id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="p-4 rounded-xl bg-white/5 border border-white/10 flex justify-between items-center hover:bg-white/10 transition">
                    <div>
                      <div className="font-medium text-pink-400">{m.nombre} <span className="text-sm text-gray-300">{m.dosis}</span></div>
                      <div className="text-sm text-gray-300">{m.descripcion}</div>
                    </div>
                    <button onClick={() => del(m._id)} className="px-3 py-1 rounded bg-rose-600/80 hover:bg-rose-700 transition">Eliminar</button>
                  </motion.div>
                ))}
              </div>
            </motion.div>
